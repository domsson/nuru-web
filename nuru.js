...
class NuruUI
{
	...
	on_files(evt)
	{
		...
		{
			let handler = this["load_" + type].bind(this, files[i].name);
	...
	load_pal(filename, evt)
	{
		console.log(evt.target);
		let buffer = evt.target.result;
		let pal = new NuruPalette(buffer);
		let name = filename.slice(0, filename.indexOf('.'));

		console.log("Loaded pal: " + name + " (type = " + pal.type + ")");

		switch (pal.type)
		{
			case 1: // 8 bit color palette
			case 3: // rgb color palette
				this.color_pals[name] = pal;
				this.add_input_opt("color-pal", name.toLowerCase(), name.toUpperCase());
				break;
			case 2: // glyph palette
				this.glyph_pals[name] = pal;
	...
	init()
	{
		...
		{
			let pal_data = new Uint8Array(this.glyph_palettes[name]);
			this.glyph_pals[name] = new NuruPalette(pal_data.buffer);
		...
		{
			let pal_data = new Uint8Array(this.color_palettes[name]);
			this.color_pals[name] = new NuruPalette(pal_data.buffer);
			if (first === null) first = name;
			this.add_input_opt("color-pal", name.toLowerCase(), name.toUpperCase());
		}
	...
	change_term_size(cols=null, rows=null, redraw=false)
	{
		...
		this.term.resize(cols, rows, glyph, fgcol, bgcol, attrs);
		if (redraw) this.redraw_term();
	}
